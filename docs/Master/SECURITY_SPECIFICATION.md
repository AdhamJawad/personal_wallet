...
## Objectives
The backend must support a small trusted-group finance product with production-grade controls. Security requirements here are designed to preserve the app's domain rules while remaining compatible with Laravel, FastAPI, Node.js, or similar stacks.

## Authentication Model

### Access Tokens
- Use short-lived JWT access tokens.
- Recommended lifetime: 10 to 15 minutes.
- Required claims:
  - `sub` user id
  - `sid` session id
...
### Refresh Tokens
- Use opaque refresh tokens or signed tokens with server-side revocation tracking.
- Store hashed refresh tokens only.
- Rotate refresh token on every refresh call.
- Revoke on logout, password reset, suspicious activity, or device removal.

### Session Binding
- Bind refresh tokens to a registered device record.
- Reject refresh from mismatched or unknown device identifiers.
- Device removal must revoke all refresh tokens bound to that device immediately.

## Password Security
- Hash passwords using Argon2id or bcrypt with a modern cost factor.
- Enforce minimum length and deny common weak passwords.
- Never log raw passwords or password-like secrets.
...
- Client biometric success should unlock a device-bound authentication ceremony, not bypass server authorization.
...
- `POST /auth/forgot-password`: 3 requests per hour per phone
...
## QR Security
- QR payload must contain only public-safe identifiers.
...
- Use secrets management for JWT keys and storage credentials.
